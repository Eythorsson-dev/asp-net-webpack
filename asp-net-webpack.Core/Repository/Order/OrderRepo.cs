...
namespace asp_net_webpack.Core.Repository.Order
{
        ...
        private string GetSql(OrderFilterRequest request, bool usePaging = false)
        {
            string sql = $@"
                SELECT 
                    ...
                    O.[PriceExVat], O.[VatTotal], O.[CreatedAt], O.[CreatedByUserId], O.[CreatedByFirstName], O.[CreatedByLastName]
            ...
            if (request.CreatedByUserId > 0) searchParam.Add("O.[CreatedByUserId] = @CreatedByUserId");
            ...
            if (!request.CreatedByFirstName.Empty()) searchParam.Add("O.[CreatedByFirstName] LIKE '%' + @CreatedByFirstName + '%'");
            if (!request.CreatedByLastName.Empty()) searchParam.Add("O.[CreatedByLastName] LIKE '%' + @CreatedByLastName + '%'");
        ...
        internal long Insert(OrderModel model)
        {
            string sql = @"
                INSERT INTO [ORDER_TAB] (
                    ...
                    [VatTotal], [CreatedAt], [CreatedByUserId], [CreatedByFirstName], [CreatedByLastName]
                )
